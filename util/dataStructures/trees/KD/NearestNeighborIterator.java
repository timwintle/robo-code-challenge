package util.dataStructures.trees.KD;


import util.dataStructures.BinaryHeap;
import util.dataStructures.IntervalHeap;
import util.dataStructures.MinHeap;
...
public class NearestNeighborIterator<T> implements Iterator<T>, Iterable<T> {
    ...
    public T next() {
        if (!hasNext()) {
            throw new IllegalStateException("NearestNeighborIterator has ended!");
        }
